...
// YENİ ADRESİNİZ İLE GÜNCELLENDİ
const String apiUrl = "https://servis-backend-ltz0.onrender.com";
String? _token;
String? _userRole;
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _saveSettings() async {
    if (!_formKey.currentState!.validate()) return;
    try {
      // DİKKAT: MultipartRequest text alanlarını json olarak değil, field olarak bekler.
      // Sadece text kaydetmek için de postMultipart benzeri bir yapı kullanılmalı
      // veya backend'de json body de kabul edilmeli.
      // Şimdilik sadece text kaydetme butonu için eski POST metodunu kullanalım.
      final response = await ApiClient.post(
        "/settings",
        body: {
                ...
                child: ListView(
                  children: [
                    ...
                    ElevatedButton(
                      ...
                      child: const Text('Sadece Bilgileri Kaydet'),
                    ),
                  ],
                ),
              ),
