...
class ApiClient {
  ...
  static http.Response _processResponse(http.Response response) {
    try {
      ...
      // Eðer decode edilemezse (örneðin binary veri ise) orijinal yanýtý döndür
      return response;
    }
  }
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final error = json.decode(utf8.decode(response.bodyBytes))['error'] ?? 'Giriþ yapýlamadý';
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error)));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sunucuya baðlanýlamadý: $e')));
    } finally {
      if (mounted) {
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(controller: _passwordController, obscureText: true, decoration: const InputDecoration(labelText: 'Þifre', border: OutlineInputBorder())),
...
class _DeviceListScreenState extends State<DeviceListScreen> {
  ...
  Future<void> fetchDevices({String? searchQuery}) async {
    ...
    try {
      ...
      if (mounted) setState(() => _error = "Sunucuya baðlanýrken bir hata oluþtu. Lütfen baðlantýnýzý kontrol edin.");
    } finally {
  ...
  void openAddForm() async {
    ...
    // Geri dönüldüðünde listeyi yenile
...
// ---------------- Device Detail Screen (TAMAMEN GÜNCELLENDÝ) ----------------
  ...
  Future<void> completeDevice() async {
    ...
    if (mounted) {
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ýþlem tamamlanamadý.')));
      }
    }
  }

  // --- YENÝ VE ÝÞLEVSEL PDF ÝNDÝRME FONKSÝYONU ---
  Future<void> downloadPdf(String endpoint, String defaultFilename) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (outputFile != null) {
          // Eðer kullanýcý uzantý girmediyse, .pdf ekle
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: const Text('PDF baþarýyla kaydedildi!'),
      ...
      final error = 'PDF indirilirken bir hata oluþtu: $e';
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(error)));
    }
  }
             ...
             IconButton(
              ...
              onPressed: () => showDialog(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: const Text("Bu kaydý tamamlandý olarak iþaretlemek istediðinizden emin misiniz?"),
                  actions: [
                    TextButton(child: const Text("Hayýr"), onPressed: () => Navigator.of(ctx).pop()),
...
// ... KODUNUZUN GERÝ KALAN TÜM SINIFLARI (AddDeviceForm, EditDeviceForm, UserManagementScreen, vs.) ...
// Bu sýnýflarda deðiþiklik yapmanýza gerek yoktur, mevcut halleriyle kalabilirler.
// Lütfen bu yorum satýrýnýn altýndaki kodlarý kendi projenizden kopyalayýp yapýþtýrýn.
// --- ADD DEVICE FORM ---
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              buildTextField("Ýletiþim Kiþisi", "iletisim_kisi"),
              buildTextField("Ýletiþim Telefonu", "iletisim_tel"),
